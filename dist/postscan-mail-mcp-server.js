...
// node_modules/ajv/node_modules/uri-js/dist/es5/uri.all.js
var require_uri_all = __commonJS({
  "node_modules/ajv/node_modules/uri-js/dist/es5/uri.all.js"(exports, module) {
    "use strict";
...
var require_fast_deep_equal = __commonJS({
  "node_modules/ajv/node_modules/fast-deep-equal/index.js"(exports, module) {
    "use strict";
...
var UnifiedSearchEngine = class {
  documentsIndex = /* @__PURE__ */ new Map();
  ...
  constructor(baseUrl = "https://www.postscanmail.com/tools/markdown/") {
  ...
  async indexRemoteDocument(filename) {
    try {
  ...
  extractMetadata(filename, content, url) {
    ...
    const fileType = filename.endsWith(".md") ? "markdown" : filename.endsWith(".json") ? "json" : "text";
    return {
      filename,
      url,
      title,
  ...
  determineCategory(filename) {
    ...
    if (name.includes("mail") || name.includes("mailing") || name.includes("service") || name.includes("virtual") || name.includes("mailroom") || name.includes("forwarding") || name.includes("receiving") || name.includes("package") || name.includes("mailbox") || name.includes("po box") || name.includes("address") || name.includes("registered") || name.includes("agent") || name.includes("virtual") || name.includes("office") || name.includes("storage")) {
      return "services";
    }
    if (name.includes("faq") || name.includes("help") || name.includes("support") || name.includes("troubleshooting") || name.includes("contact") || name.includes("issue") || name.includes("billing") || name.includes("invoice")) {
      return "support";
    }
    if ([
      "price",
    ...
    if (name.includes("setup") || name.includes("sign up") || name.includes("signup") || name.includes("registration") || name.includes("guide")) {
      return "onboarding";
    }
    return "general";
  }
  ...
  extractKeywords(content) {
    const cleanContent = content.replace(/```[\s\S]*?```/g, "").replace(/`[^`]+`/g, "").replace(/\[([^\]]+)\]\([^)]+\)/g, "$1").replace(/[#*_`\[\]()]/g, " ").toLowerCase();
    const words = cleanContent.split(/\s+/).filter((word) => word.length > 3 && !this.isStopWord(word));
  ...
  extractPricingTerms(content) {
    const pricingTerms = /* @__PURE__ */ new Set();
    const patterns = [
      /\$[\d.,]+/g,
      // Dollar amounts
      /[\d.]+\s*(?:per|\/)\s*\w+/gi,
    ...
    patterns.forEach((pattern) => {
      ...
      matches.forEach((match) => {
        const cleaned = match.toLowerCase().replace(/[^\w\s$./]/g, "").trim();
        if (cleaned.length > 2) {
  ...
  extractHeaderKeywords(content) {
    const headers = content.match(/^#{1,6}\s+(.+)$/gm) || [];
    const keywords = /* @__PURE__ */ new Set();
    headers.forEach((header) => {
      const text = header.replace(/^#+\s+/, "").toLowerCase();
      const words = text.split(/\s+/).filter((word) => word.length > 2 && !this.isStopWord(word));
      words.forEach((word) => keywords.add(word));
    });
    return Array.from(keywords);
  }
  createSummary(content) {
    const paragraphs = content.replace(/^---[\s\S]*?---/m, "").replace(/^#{1,6}\s+.*$/gm, "").split("\n\n").map((p) => p.trim()).filter((p) => p.length > 50);
  ...
  preprocessQuery(query) {
    return query.toLowerCase().replace(/[^\w\s]/g, " ").split(/\s+/).filter((term) => term.length > 2 && !this.isStopWord(term));
  }
  calculateRelevance(metadata, searchTerms, fuzzyMatch) {
    let score = 0;
    const matchedTerms = [];
    ...
    for (const term of searchTerms) {
      ...
      for (const keyword of metadata.keywords) {
        if (keyword.includes(term) || fuzzyMatch && this.fuzzyMatch(keyword, term)) {
          termScore += 15;
  ...
  generateSnippet(content, searchTerms) {
    ...
    if (allSnippets.length === 0) {
      ...
      for (let i = 0; i < sentences.length - 1; i++) {
        const snippet = sentences.slice(i, i + 3).join(". ").trim();
    ...
    let result = topSnippets.map((snippet) => this.highlightTerms(snippet.text, searchTerms)).join("\n\n---\n\n");
    if (result.length > 1500) {
      result = result.substring(0, 1500) + "...";
    }
    return result;
  }
  findRelevantSections(content, searchTerms) {
    const sections = [];
      ...
      if (line.match(/^#{1,6}\s+/) || line.match(/^[A-Z][^a-z]*$/)) {
        ...
        if (headerScore > 0) {
          ...
          while (j < lines.length) {
            ...
            const currentHeaderLevel = (line.match(/^(#+)/) || ["", ""])[1].length;
            ...
            } else if (nextLine.match(/^[A-Z][^a-z]*$/) && nextLine.length > 3 && !nextLine.includes("$") && currentHeaderLevel <= 2) {
              break;
            }
          ...
          sections.push({
            ...
            score: headerScore + this.calculateSnippetScore(sectionContent, searchTerms)
          });
        }
      }
    }
  ...
  calculateSnippetScore(text, searchTerms) {
    let score = 0;
    ...
    this.server.setRequestHandler(ListResourcesRequestSchema, async () => {
      ...
      return {
        resources: allDocs.map((doc) => ({
          ...
          mimeType: doc.fileType === "markdown" ? "text/markdown" : "text/plain",
          name: doc.title,
          ...
          contents: [
            {
              ...
              mimeType: metadata.fileType === "markdown" ? "text/markdown" : "text/plain",
              text: content,
              annotations: {
                title: metadata.title,
                category: metadata.category,
...
\u{1F3AF} PURPOSE: This tool searches PostScan Mail's official documentation. You MUST follow these rules:

\u{1F4CB} BEFORE USING THIS TOOL:
...
- Suggest contacting support for topics not covered in documentation`,
            inputSchema: {
              ...
              properties: {
                query: {
                  type: "string",
                  description: "Specific search query based on user intent analysis. Use clear, relevant keywords.",
                  minLength: 3,
                  maxLength: 200
                },
                category: {
                ...
                intent_analysis: {
                  type: "string",
                  description: "Brief explanation of detected user intent (for logging/debugging)",
                  maxLength: 100
                },
                confidence_level: {
                  type: "string",
            ...
            inputSchema: {
              ...
              properties: {
                ...
                clarification_needed: {
                  ...
                  description: "List of specific things that need clarification",
                  minItems: 1,
                  maxItems: 5
                },
                suggested_questions: {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                search_results_summary: {
                  type: "string",
                  description: "Summary of what was actually found in search results"
                },
                confidence_check: {
                  type: "string",
                  enum: ["verified", "partially_verified", "unverified"],
                  description: "How well your response is supported by search results"
                }
              },
              required: [
                "planned_response",
...
Use this when you need to fetch the latest content from the remote documentation.`,
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          ...
          case "refresh_cache":
            return await this.handleRefreshCache(args);
          default:
            throw new McpError(
  ...
  async handleIntelligentSearch(args) {
    const gibberishAnalysis = this.analyzeQueryForGibberish(args.query);
    if (gibberishAnalysis.isGibberish) {
      console.error(
        `[INFO] Gibberish detected: ${args.query} (${(gibberishAnalysis.gibberishRatio * 100).toFixed(1)}% gibberish) - Using existing escalation system`
      );
      return await this.handleEscalateToHuman({
        user_question: args.query,
        searches_attempted: ["gibberish_detected"],
        escalation_reason: "no_documentation_found",
        contact_recommendation: "live_chat"
      });
    }
    console.error(
  ...
  async handleClarifyIntent(args) {
...
This will help me search our documentation more effectively and give you the most accurate information.`;
  ...
  async handleValidateResponse(args) {
...
`;
    if (args.confidence_check === "unverified") {
      validation += `\u{1F6A8} **WARNING:** Response contains unverified information. Do not provide this response.
`;
...
`;
  ...
  async handleEscalateToHuman(args) {
...
`;
    switch (args.contact_recommendation) {
      case "email_support":
        response += `\u{1F4E7} **Email Support:** support@postscanmail.com
`;
...
`;
        break;
      case "phone_support":
        response += `\u{1F4DE} **Phone Support:** 1-800-XXX-XXXX
`;
...
`;
        break;
      case "live_chat":
        response += `\u{1F4AC} **Live Chat:** Available on our website
`;
        response += `\u2022 Monday-Friday, 9AM-6PM EST
`;
...
`;
        break;
      case "help_center":
        response += `\u{1F4CB} **Help Center:** Visit our website's help section
`;
...
They'll have access to your account details and can provide personalized assistance.`;
  ...
  async handleRefreshCache(_args) {
    try {
...
Please try restarting the server if the problem persists.`
  ...
  // === GIBBERISH DETECTION METHODS ===
  analyzeQueryForGibberish(query) {
    const words = query.toLowerCase().split(/\s+/).filter((w) => w.length > 2);
    const gibberishWords = [];
    for (const word of words) {
      const result = this.isGibberish(word);
      if (result.isGibberish) {
        gibberishWords.push({ word, reason: result.reason });
      }
    }
    const gibberishRatio = gibberishWords.length / words.length;
    return {
      isGibberish: gibberishRatio > 0.5,
      gibberishWords,
      gibberishRatio,
  ...
  isGibberish(word) {
    ...
    for (const row of keyboardRows) {
      for (let direction of [row, row.split("").reverse().join("")]) {
        for (let i = 0; i <= direction.length - normalizedWord.length; i++) {
          if (normalizedWord.length >= 4 && direction.substring(i, i + normalizedWord.length) === normalizedWord) {
            return {
              isGibberish: true,
              reason: `keyboard_pattern_${direction.substring(i, i + normalizedWord.length)}`
            };
          }
        }
      }
    ...
    if (maxCharFreq / normalizedWord.length > 0.5) {
      return { isGibberish: true, reason: "excessive_repetition" };
    }
    if (normalizedWord.length > 3) {
      const vowels = /[aeiou]/;
      if (!vowels.test(normalizedWord)) {
        const validAcronyms = [
          "llc",
          "inc",
          "ltd",
        ...
        if (!validAcronyms.includes(normalizedWord)) {
          return { isGibberish: true, reason: "no_vowels" };
        }
      }
    }
    if (/[bcdfghjklmnpqrstvwxyz]{5,}/.test(normalizedWord)) {
      return { isGibberish: true, reason: "consonant_cluster" };
    }
    if (/(.)\1{2,}/.test(normalizedWord)) {
      return { isGibberish: true, reason: "triple_characters" };
    }
    return { isGibberish: false, reason: null };
  }
  // === ENHANCED RESULT FORMATTING ===
  formatIntelligentSearchResults(args, results) {
    if (results.length === 0) {
      return this.formatNoResultsFound(args);
    }
    let output = `**\u{1F50D} Search Results for "${args.query}"** (Category: ${args.category})

`;
    const confidenceEmoji = args.confidence_level === "high" ? "\u2705" : args.confidence_level === "medium" ? "\u26A0\uFE0F" : "\u2753";
...
`;
      ...
      output += `*\u{1F4C4} Source: ${result.filename}*
`;
      output += `---
...
`;
    ...
    if (briefMentions.length > 0) {
      output += `**\u{1F4CB} Additional Related Documents (Brief):**
...
`;
    output += `\u{1F6AB} **I cannot provide information that isn't in our documentation** - this ensures you get accurate, up-to-date details from our support team.`;
    return output;
  }
  createRelevanceBar(score) {
...
var server = new UnifiedMCPServer();
